...
export default class EntityWithPaginationAndDTOService {
  public find(id: number): Promise<IEntityWithPaginationAndDTO> {
    return new Promise<IEntityWithPaginationAndDTO>((resolve, reject) => {
      axios
        .get(`${baseApiUrl}/${id}`)
        .then(res => {
          resolve(res.data);
  ...
  public partialUpdate(entity: IEntityWithPaginationAndDTO): Promise<IEntityWithPaginationAndDTO> {
    return new Promise<IEntityWithPaginationAndDTO>((resolve, reject) => {
      axios
