import axios from 'axios';

import { IFieldTestServiceImplEntity } from '@/shared/model/field-test-service-impl-entity.model';

const baseApiUrl = 'api/field-test-service-impl-entities';

export default class FieldTestServiceImplEntityService {
  public find(id: number): Promise<IFieldTestServiceImplEntity> {
    return new Promise<IFieldTestServiceImplEntity>((resolve, reject) => {
      axios
        .get(`${baseApiUrl}/${id}`)
        .then(res => {
          resolve(res.data);
        })
        .catch(err => {
          reject(err);
        });
    });
  }

  public retrieve(): Promise<any> {
    return new Promise<any>((resolve, reject) => {
      axios
        .get(baseApiUrl)
        .then(res => {
          resolve(res);
        })
        .catch(err => {
          reject(err);
        });
    });
  }

  public delete(id: number): Promise<any> {
    return new Promise<any>((resolve, reject) => {
      axios
        .delete(`${baseApiUrl}/${id}`)
        .then(res => {
          resolve(res);
        })
        .catch(err => {
          reject(err);
        });
    });
  }

  public create(entity: IFieldTestServiceImplEntity): Promise<IFieldTestServiceImplEntity> {
    return new Promise<IFieldTestServiceImplEntity>((resolve, reject) => {
      axios
        .post(`${baseApiUrl}`, entity)
        .then(res => {
          resolve(res.data);
        })
        .catch(err => {
          reject(err);
        });
    });
  }

  public update(entity: IFieldTestServiceImplEntity): Promise<IFieldTestServiceImplEntity> {
    return new Promise<IFieldTestServiceImplEntity>((resolve, reject) => {
      axios
        .put(`${baseApiUrl}/${entity.id}`, entity)
        .then(res => {
          resolve(res.data);
        })
        .catch(err => {
          reject(err);
        });
    });
  }

  public partialUpdate(entity: IFieldTestServiceImplEntity): Promise<IFieldTestServiceImplEntity> {
    return new Promise<IFieldTestServiceImplEntity>((resolve, reject) => {
      axios
        .patch(`${baseApiUrl}/${entity.id}`, entity)
        .then(res => {
          resolve(res.data);
        })
        .catch(err => {
          reject(err);
        });
    });
  }
}
