  ...
  <div>
    <h2>
      ...
      <div class="d-flex justify-content-end">
        ...
        <router-link custom v-slot="{ navigate }" :to="{ name: 'UserCreate' }">
      ...
      <table class="table table-striped" aria-describedby="Users">
        ...
        <tbody v-if="users">
          <tr v-for="user in users" :key="user.id" :id="user.login">
            <td>
              <router-link :to="{ name: 'UserView', params: { userId: user.login } }">{{ user.id }}</router-link>
            </td>
            <td>{{ user.login }}</td>
            ...
            <td class="text-right">
              <div class="btn-group">
                <router-link :to="{ name: 'UserView', params: { userId: user.login } }" custom v-slot="{ navigate }">
                  <button @click="navigate" class="btn btn-info btn-sm details">
                    <font-awesome-icon icon="eye"></font-awesome-icon>
                    <span class="d-none d-md-inline" v-text="$t('entity.action.view')">View</span>
                  </button>
                </router-link>
                <router-link :to="{ name: 'UserEdit', params: { userId: user.login } }" custom v-slot="{ navigate }">
